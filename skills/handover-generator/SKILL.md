...
1. `.handover_workspace/` 폴더를 생성한다.
2. 사용자가 제공한 경로(및 하위 디렉토리)를 재귀 스캔한다.
3. Node.js 스크립트를 작성하여 파일명에서 정규표현식으로 메타데이터를 추출한다:

```javascript
정규표현식: /\(([^-]+)-(\d+)\s+\((본문|첨부)\)\)/

// ⚠️ 제목 추출 시 주의: 마지막 괄호 ")" 다음부터 추출해야 함
const afterPattern = fileName.lastIndexOf(')') + 1;  // 첫 번째가 아닌 마지막!
const titleWithExt = fileName.substring(afterPattern).trim();
const title = titleWithExt.substring(0, titleWithExt.lastIndexOf('.'));
```

추출 필드:
- **기관명** (organization): 그룹 1 (예: `인창고등학교`)
- **공문번호** (docNumber): 그룹 2 (예: `22206`)
- **문서유형** (docType): 그룹 3 (`본문` 또는 `첨부`)
- **제목** (title): 마지막 괄호 `)` 뒤의 텍스트 (확장자 제외)

4. 패턴 미매칭 파일도 별도로 기록한다 (수동 확인 대상).
5. **연도 그룹 분리**를 수행한다 (아래 참조).
...
2. 같은 공문번호의 본문+첨부를 그룹핑한다.
   ...
   **⚠️ 중요:** 그룹핑 시 `organization` 필드도 반드시 포함해야 한다:
   ```javascript
   docGroups[key] = {
     organization: doc.organization,  // ← 필수! 누락하면 "undefined" 출력
...
- 중간 산출물은 모두 `.handover_workspace/`에 격리
