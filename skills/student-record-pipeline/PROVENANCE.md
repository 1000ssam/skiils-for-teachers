...
| **`achievement-standards-2022.csv`** | 성취기준 원문 3,285건(초653·중580·고2052 / 160과목) — `standards_backing` 조회용 | **2022 개정 교육과정 성취기준(교육부 고시) = 공공데이터.** 취합본을 `pblsketch/gepai-mcp`(MIT) `data/source/achievement-standards.csv`에서 가져와 **무가공 그대로** 둠(2026-07-29). 컬럼=학교·과목·학년(학년군)·성취기준 코드·성취기준 내용 |
| **`standards.py`** | 위 CSV 조회(과목·코드·키워드·학교급). 오프라인·외부 의존 0 | 파생 도구(원천 없음). 🚩 코드는 유일키 아님(`[12스문]`=스포츠 문화/스페인어권 문화) · 🚩 2022 개정 개명 과목 존재(동아시아사→`동아시아 역사 기행`) |
