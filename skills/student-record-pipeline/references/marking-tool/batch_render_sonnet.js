...
/* @@ARGS-INJECTION-POINT@@ — build_run_batch.py 가 이 줄을 `const args = {…}` 로 바꿔
   ...
   이 marker 위는 meta 뿐이어야 한다 — 워크플로 스크립트는 meta 로 시작해야 하기 때문이다. */
...
const SCHEMA = {
  ...
  properties: {
    ...
    /* 교사 기타 요구사항을 못 지켰을 때의 사유 — 교사에게 **직접 보이는** 칸이다(교사 요청 2026-07-31).
       notes 는 표면 처리 메모라 마킹 화면엔 안 뜬다(다만 검수 엑셀 '확인할 것' 칸에는 실린다).
       요구사항을 물리는 건 교사가 곧바로 알아야 할 판단이므로 별도 칸으로 뽑아 화면 상단
       배너로 띄운다. 지킨 경우엔 빈 문자열. */
    unmet: { type: 'string' },
  },
...
function rejectBlock(s) {
  /* 🔑 **사유 없는 반려는 정보가 아니다.**
     선생님이 까닭을 안 적고 [다시]를 누르신 것은 "같은 조건에서 한 번 더 돌려라"는 뜻일 뿐이다.
     그걸 '이 문장이 틀렸다' · '더 채워라' · '덜어내라' 로 읽으면 **없는 지시를 지어내는 것**이다.
     실제로 그 오독 때문에 재작성이 395B → 415B 로 되레 길어졌다(실측 2026-08-03).
     → 사유가 적힌 반려만 싣는다. 하나도 없으면 이 절이 통째로 빠져 **처음 쓸 때와 똑같은
       프롬프트**가 되고, 달라지는 것은 표집의 우연뿐이다. 그게 선생님이 요청하신 것이다. */
  const rs = (s.rejects || []).filter(r => (r.reason || '').trim())
  if (!rs.length) return ''
  ...
  const reasons = rs.map((r, i) =>
    `  ${i + 1}. ${r.reason.trim()}` + (r.setuk ? `\n     ↳ 물린 본문: "${r.setuk}"` : '')
  ).join('\n')
  const prev = (!hasSnap && s.prev_setuk)
    ? `\n직전 초안(교사가 물린 문장 — 이 문장을 그대로 다시 쓰지 마라):\n"${s.prev_setuk}"` : ''
  const thin = rs.length >= 3
    ? '\n※ 까닭을 적어 3회 이상 물리셨다. 표현을 더 꾸미지 마라 — 더 짧고 정확하게 써라.'
    : ''
  return `\n**교사 반려 이력(까닭 적힌 것 ${rs.length}회) — 다음 규칙보다 우선한다:**\n${reasons}${prev}${thin}\n`
}
...
도달 수준 규칙(계약서 §1-2): 상=도달까지 쓴다 / 중=도달을 한 겹만 / 하=한 일 서술로 끝낸다 / 미지정=재료 두께로 추정. **어느 낱말을 쓸지는 정해져 있지 않다 — 계약서 §3 퓨샷의 같은 레벨 예시가 그 문체를 보여준다. 그 결을 따르라.** 레벨은 부풀리기 허가가 아니다 — 재료가 없으면 상이어도 짓지 말고 unmet에 쉬운 말로 남겨라.
...
/* 상한 근처는 거의 항상 패딩이다(계약서 §2 실측 사고). 본문을 안 싣는 대신 이 신호는 올린다.
   기준은 이 배치의 상한에서 뽑는다 — 예전엔 770B 가 박혀 있어서, 상한이 그보다 낮은 배치
   (예: 650B)에서는 아무도 안 걸리는 죽은 코드였다. byte_max 는 check_config.py 가 미리 막는다. */
const PAD_NEAR = SURFACE.byte_max ? Math.round(SURFACE.byte_max * 0.95) : null
const padded = PAD_NEAR ? results.filter(r => r && !r.error && r.bytes_est >= PAD_NEAR).map(r => `${r.hakbun} ${r.name}(${r.bytes_est}B)`) : []
if (padded.length) log(`🚩 상한 ${SURFACE.byte_max}B 에 바짝 붙은 ${padded.length}명(${PAD_NEAR}B 이상) — 패딩 의심, 화면에서 먼저 볼 것: ${padded.join(', ')}`)
else if (!PAD_NEAR) log('ℹ️ 바이트 상한이 없어 패딩 검사를 건너뜁니다')
