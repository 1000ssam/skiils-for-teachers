...
```
Glob → 대상 경로 하위 모든 파일 목록 수집
```

1. 사용자가 제공한 경로(및 하위 디렉토리)를 재귀 스캔한다.
2. 파일명에서 정규표현식으로 메타데이터를 추출한다:
...
1. 같은 공문번호의 본문+첨부를 그룹핑한다.
2. Phase 1에서 부여한 **연도 라벨 → 공문번호** 순으로 정렬하여 업무 흐름을 파악한다.
3. 공문 제목의 의미를 분석하여 업무 카테고리를 자율적으로 도출한다.
4. 반복 패턴을 식별한다 (계획→수정→재수정, 위원회 구성→개회→결과 등).
5. 인접 공문번호의 문서들을 묶어 맥락을 보강한다.
...
```
Task(Explore) → 원본 파일 목록 vs 인수인계서 내 공문번호 비교
```

1. Phase 1에서 수집한 전체 공문번호 목록을 추출한다.
2. 작성된 인수인계서에서 언급된 공문번호를 추출한다.
...
| 사용자 입력 | 동작 |
|---|---|
| `"인수인계서 작성"` / `"인수인계서 만들어줘"` | 경로 확인 후 전체 워크플로 실행 |
