...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="문장 만연도·주술관계 표면 신호(advisory) — 후보만 지목하고 재구성은 LLM 몫(D3 의미축).",
        epilog="인자를 주지 않으면 stdin 을 읽는다.  예) python3 sentence_metrics.py out/drafts")
    ap.add_argument("paths", nargs="*", help="초안 txt 파일 또는 폴더(재귀)")
    args = ap.parse_args()
